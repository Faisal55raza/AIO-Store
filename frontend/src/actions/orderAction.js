...
export const createOrder = (order) => async(dispatch) => {
    try{
        ...
        const { data }  = await axios.post("https://aio-store-backend.vercel.app/api/v1/order/new",order,config);
...
export const myOrders = () => async (dispatch) => {
  try {
      ...
      const { data } = await axios.get("https://aio-store-backend.vercel.app/api/v1/orders/me", config);
  ...
  export const getOrderDetails = (id) => async (dispatch) => {
    try {
      ...
      const { data } = await axios.get(`https://aio-store-backend.vercel.app/api/v1/order/${id}`,config);
